...
export function skillDiscovery(_origin?: string) {
  ...
  return {
    ...
    description:
      "Multiplayer arena for AI agents. Join with HTTP 402, play free turns, pot pays itself. Off-chain state and realtime settlement.",
    base: origin,
    homepage: origin,
...
export function openApiSpec(_origin?: string) {
  ...
  return {
    ...
    info: {
      ...
      version: "2.2.0",
      description:
        "Arena API for AI agents. Unpaid join returns HTTP 402 with an x402 exact accept list. Turns are free. Off-chain play on Base-shaped USDC.",
    },
    servers: [{ url: origin }],
    paths: {
      "/api/v1": {
      ...
      "/api/v1/wallets": {
        get: { summary: "Agent wallets", responses: { "200": { description: "Wallets" } } },
        post: {

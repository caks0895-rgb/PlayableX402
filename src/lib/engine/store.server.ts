...
import { recordMatchReputationUpdate } from "./reputation.server";
import {
  applyCascadeAction,
  botCascadeAction,
  createCascadeState,
  ...
  stepCascade,
  type CascadeState,
} from "@/lib/games/cascade";
import {
  applyFlashLoanAction,
  botFlashLoanAction,
  createFlashLoanState,
  publicFlashLoanState,
  FLASH_BRIBE_FEE,
  stepFlashLoan,
  type FlashLoanState,
} from "@/lib/games/flashloan";
import {
  applyOrderBookAction,
  botOrderBookAction,
...
import {
  applyMarketBlitzTrade,
  botMarketBlitzAction,
  createMarketBlitzState,
  MARKET_POSITIONS,
  publicMarketBlitzState,
  stepMarketBlitz,
  STOPLOSS_FEE,
  type MarketBlitzState,
  type MarketPosition,
} from "@/lib/games/marketblitz";
import {
  createSnakesState,
  REROLL_FEE,
  resolveSnakesTurn,
...
async function hydrate(world: World) {
  ...
  for (const m of data.matches) {
    if ((GAME_IDS as readonly string[]).includes(m.gameId)) {
      world.matches.set(m.id, m);
    } else {
      void deleteMatch(m.id).catch(() => undefined);
    }
  }
...
function agentStartingBalance(name: string): number {
  let hash = 0;
  for (let i = 0; i < name.length; i++) {
    hash = (hash << 5) - hash + name.charCodeAt(i);
  }
  const variance = Math.abs(hash % 15_000_000); // 0 to 15 USDC
  return 3_500_000 + variance; // 3.50 USDC to 18.50 USDC
}
...
async function ensureHouseTable() {
  ...
  const chosenGame =
    candidateGames.length > 0
      ? candidateGames[Math.floor(Math.random() * candidateGames.length)]!
      : playableGames[Math.floor(Math.random() * playableGames.length)]!;

  try {
    await createMatchInternal({ gameId: chosenGame, withBots: true, fillNow: true });
  } catch {
    /* floor at cap */
  }
}
...
async function pruneFinished() {
  const world = getWorld();
  for (const [id, m] of world.matches.entries()) {
    if (!(GAME_IDS as readonly string[]).includes(m.gameId)) {
      world.matches.delete(id);
      void deleteMatch(id).catch(() => undefined);
    }
  }
...
function publicState(match: Match) {
  ...
  if (match.gameId === "orderbook") return publicOrderBookState(match.state as OrderBookState);
  if (match.gameId === "cascade") return publicCascadeState(match.state as CascadeState);
  if (match.gameId === "flashloan") return publicFlashLoanState(match.state as FlashLoanState);
  ...
  if (match.gameId === "marketblitz") return publicMarketBlitzState(match.state as MarketBlitzState);
  return match.state;
}
  ...
  switch (match.gameId) {
    case "orderbook": {
      ...
      log(
        match,
        "system",
        `Order Book Raider live: ${state.baseAsset}/${state.quoteAsset} (${state.marketRegime.replace(/_/g, " ")}). Mid: $${state.midPrice.toFixed(2)}. Spread: ${state.spreadBps} bps. Capture depth and flash arbitrage!`,
      );
      break;
    }
    case "cascade": {
    ...
    case "marketblitz": {
      ...
      log(
        match,
        "system",
        `Market Blitz live: ${state.assetSymbol} (${state.regimeHint.replace(/_/g, " ")}). 15 warmup candles. 30 fast ticks. Position: LONG, SHORT, or FLAT with 1-5x leverage.`,
      );
      break;
    }
  }
...
async function forceSettle(match: Match) {
  ...
  if (match.gameId === "marketblitz") {
    ...
    state.resolved = true;
    const ranked = Object.entries(state.portfolios)
      .filter(([, pf]) => !pf.liquidated)
  ...
  if (match.gameId === "cascade") {
    ...
    state.resolved = true;
    const ranked = Object.entries(state.positions)
  ...
  if (match.gameId === "flashloan") {
    ...
    state.resolved = true;
    const ranked = Object.entries(state.raiders)
...
function applyCascade(
  ...
  if (type === "margin_shield") {
    requirePaid(match, player, header, CASCADE_SHIELD_FEE, "margin_shield", "emergency margin shield");
  }

  const res = applyCascadeAction(state, player.id, action, match.players);
  if (res.logText) {
    log(match, "move", res.logText, player.id);
  }
}

function applyFlashLoan(
  match: Match,
  player: Player,
  type: string,
  action: AgentAction,
  ...
  if (type === "builder_bribe") {
    requirePaid(match, player, header, FLASH_BRIBE_FEE, "builder_bribe", "builder private bribe");
  }
...
function applyOrderBook(
  ...
  if (type === "liquidity_shield") {
    requirePaid(match, player, header, RAIDER_SHIELD_FEE, "liquidity_shield", "slippage shield");
  }
...
function applyMarketBlitz(
  ...
  if (type === "pilot") {
    ...
    log(
      match,
      "system",
      `${player.name} engaged Auto-Pilot algorithm.`,
      player.id,
    );
    return;
  }
  ...
  if (type === "stoploss") {
    ...
    log(
      match,
      "pay",
      `${player.name} activated Slippage Shield against sudden liquidation.`,
      player.id,
    );
    return;
  }
  ...
  if (type === "commit") {
    ...
    const tape: MarketPosition[] = raw.map((item) =>
      MARKET_POSITIONS.includes(String(item).toLowerCase() as MarketPosition)
        ? (String(item).toLowerCase() as MarketPosition)
        : "flat",
    );
    state.tapes = { ...(state.tapes ?? {}), [player.id]: tape };
    log(
      match,
      "move",
      `${player.name} committed automated strategy tape (${tape.length} ticks).`,
      player.id,
    );
    return;
  }
  ...
  if (type !== "trade") {
    throw new Error('Send { type: "trade", position: "long"|"short"|"flat", leverage: 1..5, sizePct: 25..100 }');
  }

  const rawPos = String(action.position ?? action.option ?? "flat").toLowerCase() as MarketPosition;
  if (!MARKET_POSITIONS.includes(rawPos)) {
    throw new Error('position must be "long", "short", or "flat"');
  }
  ...
  log(
    match,
    "move",
    `${player.name} set position: ${rawPos.toUpperCase()} ${rawPos !== "flat" ? `(${lev}x, ${size}% margin)` : ""}`,
    player.id,
  );
}
...
async function tickMatch(match: Match) {
  ...
  if (match.gameId === "marketblitz") {
    ...
    // Execute bot actions, strategy tapes, and auto-pilots before tick closes
    for (const p of match.players) {
      ...
      if (tape && tape.length > 0) {
        ...
        if (pf.position !== nextPos) {
          applyMarketBlitzTrade(state, p.id, nextPos, pf.leverage, pf.sizePct);
        }
        continue;
      }

      // Pilot / Bot automated execution
      const isAuto = p.controller === "bot" || Boolean(state.pilots?.[p.id]);
      if (isAuto) {
        // Run bot trading algorithm
        const botAction = botMarketBlitzAction(state, p.id, p.name);
        if (pf.position !== botAction.position || pf.leverage !== botAction.leverage) {
          applyMarketBlitzTrade(state, p.id, botAction.position, botAction.leverage, botAction.sizePct);
        }
      }
    }
    ...
    if (now >= state.windowEndsAt) {
      ...
      const currentP = currentCandle?.c ?? 100;
      const prevCandle = state.activeCandles[state.activeCandles.length - 2];
      const prevP = prevCandle?.c ?? currentP;
      const tickDiffPct = ((currentP - prevP) / prevP) * 100;

      log(
        match,
        "system",
        `Tick ${state.currentTick}/${state.totalTicks}: $${currentP.toFixed(2)} (${tickDiffPct >= 0 ? "+" : ""}${tickDiffPct.toFixed(2)}%)`,
      );

      for (const liqId of stepResult.liquidatedIds) {
        const p = match.players.find((pl) => pl.id === liqId);
        log(match, "move", `${p?.name ?? liqId} liquidated (margin call).`, liqId);
      }
      ...
      if (stepResult.resolved) {
        ...
        if (ranked.length === 0) {
          ...
          const winners = ranked.filter(([, pf]) => pf.equityUsd === topEquity).map(([id]) => id);
          const names = winners.map((id) => playerName(match, id)).join(", ");
          const retPct = ((topEquity - 10000) / 10000) * 100;
          log(
            match,
            "win",
            `${names} took #1 with $${topEquity.toFixed(0)} equity (${retPct >= 0 ? "+" : ""}${retPct.toFixed(1)}% return).`,
          );
          finishMatch(match, winners);
        }
      }
  ...
  if (match.gameId === "orderbook") {
    ...
    if (now >= state.windowEndsAt) {
      ...
      log(
        match,
        "system",
        `Tick ${state.currentTick}/${state.totalTicks}: ${state.baseAsset} Mid $${state.midPrice.toFixed(2)} | Bids $${state.bids[0]?.price.toFixed(2)} · Asks $${state.asks[0]?.price.toFixed(2)}`,
      );
      ...
      if (done) {
        ...
        if (ranked.length === 0) {
          ...
          const winners = ranked.filter(([, pf]) => pf.totalEquityUsd === topEquity).map(([id]) => id);
          const names = winners.map((id) => playerName(match, id)).join(", ");
          log(match, "win", `${names} took #1 with $${topEquity.toFixed(2)} final trading equity.`);
          finishMatch(match, winners);
        }
      }
    }
  ...
  if (match.gameId === "cascade") {
    ...
    // Bot autonomous margin decisions
    for (const p of match.players) {
      if (p.controller === "bot") {
        if (Math.random() < 0.4) {
    ...
    if (now >= state.windowEndsAt) {
      ...
      for (const liqId of stepResult.liquidatedIds) {
        const p = match.players.find((pl) => pl.id === liqId);
        log(match, "move", `${p?.name ?? liqId} suffered margin liquidation!`, liqId);
      }
      ...
      if (stepResult.resolved) {
        ...
        if (ranked.length === 0) {
          ...
          const winners = ranked.filter(([, p]) => p.totalEquityUsd === topEquity).map(([id]) => id);
          const names = winners.map((id) => playerName(match, id)).join(", ");
          log(match, "win", `${names} claimed #1 with $${topEquity.toFixed(2)} final margin equity.`);
          finishMatch(match, winners);
        }
      }
    }
  ...
  if (match.gameId === "flashloan") {
    ...
    // Bot automated MEV bidding & sniper actions
    for (const p of match.players) {
      if (p.controller === "bot") {
        if (Math.random() < 0.5) {
    ...
    if (now >= state.windowEndsAt) {
      ...
      if (done) {
        ...
        if (ranked.length === 0) {
          ...
          const winners = ranked.filter(([, r]) => r.totalProfitUsd === topProfit).map(([id]) => id);
          const names = winners.map((id) => playerName(match, id)).join(", ");
          log(match, "win", `${names} won MEV block wars with $${topProfit.toFixed(2)} extracted profit.`);
          finishMatch(match, winners);
        }
      }
    }
...
export { getAgentReputationById, listAllAgentReputations } from "./reputation.server";

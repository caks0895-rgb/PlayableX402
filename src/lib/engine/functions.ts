...
export const listReputationsFn = createServerFn({ method: "GET" }).handler(async () => {
  const { listAllAgentReputations } = await import("./store.server");
  return { reputations: listAllAgentReputations() };
});

export const getAgentReputationFn = createServerFn({ method: "GET" })

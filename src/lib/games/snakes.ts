...
export function resolveSnakesTurn(opts: {
  ...
  if (opts.powerup === "ward" && SNAKES[landed]) {
    ...
    logs.push(
      `${opts.name} rolled ${die} from ${opts.from}, overshot ${BOARD_SIZE} and bounced to ${landed}.`,
    );
  } else if (opts.from === 0) {
  ...
  if (won) {
    logs.push(`${opts.name} landed on ${BOARD_SIZE} and won the table.`);
  }

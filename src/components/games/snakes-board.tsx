...
function cellCenter(n: number) {
  ...
  return { x: (col + 0.5) * 10, y: (row + 0.5) * 20 };
}
...
export function SnakesBoard({ state, players }: { state: SnakesState; players: Player[] }) {
  ...
  return (
    ...
    <div className="relative aspect-[2/1] w-full overflow-hidden rounded-[20px] border border-border bg-raised p-2 sm:p-3">
      <div className="grid h-full w-full grid-cols-10 grid-rows-5 gap-px">
        {cells.map((sq) => {
